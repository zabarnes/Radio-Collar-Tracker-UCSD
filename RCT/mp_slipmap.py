...
from lib.mp_menu import *
from lib import mp_widgets
...
class MainFrame(wx.Frame):
    ...
    def __init__(self,parent,id,title,position,size, state):
        ...
        self.rightpanel = MPSlipMapFrame(state=state,parent=self.split1)
        self.leftpanel = MPInfoPanel(parent=self.split1,state=state)
...
class MPInfoPanel(wx.Panel):
    ...
    def __init__(self, parent, state):
        ...
        self.toppanel = ProcPanel(self, state)
        #self.button1 = wx.Button(self.bitmap1, id=-1, label='Button1', pos=(8, 8))
class ProcPanel(wx.Panel):
    def __init__(self,parent,state):
        global screenSize
        self.state=state
        panelLength= screenSize[1] - 102
        self.heatTrue = 1
        wx.Panel.__init__(self, parent, pos=(12,64),size=(460,panelLength))
        self.OpenSD()

        self.filelabel = wx.StaticText(self, label="FILE I/O",pos=(0,2))
        self.datalabel = wx.StaticText(self, label="PROCESSING",pos=(0,202))
        self.vislabel = wx.StaticText(self, label="VISUALIZATION TOOLS",pos=(0,325))
        self.filelabel.SetForegroundColour(wx.RED)
        self.datalabel.SetForegroundColour(wx.RED)
        self.vislabel.SetForegroundColour(wx.RED)
        #self.filelabel = wx.StaticBox(self, label="FILE I/O"
        self.line1 = wx.StaticLine(self, id=wx.ID_ANY, size=(100,-1))
        self.line2 = wx.StaticLine(self, id=wx.ID_ANY, size=(100,-1))
        self.line3 = wx.StaticLine(self, id=wx.ID_ANY, size=(200,-1))
        self.line11 = wx.StaticLine(self, id=wx.ID_ANY, size=(100,-1))
        self.line12 = wx.StaticLine(self, id=wx.ID_ANY, size=(100,-1))
        self.line13 = wx.StaticLine(self, id=wx.ID_ANY, size=(200,-1))
        self.line21 = wx.StaticLine(self, id=wx.ID_ANY, size=(100,-1))
        self.line22 = wx.StaticLine(self, id=wx.ID_ANY, size=(100,-1))
        self.line23 = wx.StaticLine(self, id=wx.ID_ANY, size=(200,-1))
        #self.fileText = wx.StaticText(self, label="File I/O", size=(100,100), pos=(5,50)) 

        self.runlabel = wx.StaticText(self, label="Run Number: ")
        self.runNum = wx.TextCtrl(self, size=(100, -1), value="RUN NUM FROM FILE")
        
        self.collarLabel = wx.StaticText(self, label="Selected Collars:")
        self.collarList = wx.StaticText(self, label="",size=(100,-1))
        self.collarList.SetForegroundColour(wx.RED)

        self.collarList2 = wx.StaticText(self, label="",size=(100,-1))
        self.collarList2.SetForegroundColour(wx.RED)

        self.collarList3 = wx.StaticText(self, label="",size=(100,-1))
        self.collarList3.SetForegroundColour(wx.RED)

        self.collarNum = wx.StaticText(self, label="Num of Collars:")
        self.collarNumPicker=wx.SpinCtrl(self, -1, '1', size=(100, -1), min=1, max=100)
        #self.collarNumPicker = wx.TextCtrl(self, size=(140, -1), value="COLLAR NUM FROM FILE")
        self.collarbutton = wx.Button(self, label="Update Collar Frequencies", size=(200,-1))

        self.write = wx.Button(self, label="Write Files")
        self.advanced = wx.Button(self, label="Advanced Settings",size=(200,-1))

        self.timer = wx.Timer(self, 1)
        self.count = 0
        self.Bind(wx.EVT_TIMER, self.OnTimer, self.timer)

        self.gauge = wx.Gauge(self, -1, 50, size=(200, 25))
        self.btn1 = wx.Button(self, label="Run", size=(100,-1))
        self.btn2 = wx.Button(self, wx.ID_STOP, size=(100,-1))
        self.text = wx.StaticText(self, -1, 'Data to be processed')

        self.graphbutton = wx.Button(self, label="Toggle Heat Map", size=(200,-1))
        self.approxbutton = wx.Button(self, label="Toggle Approximate Location", size=(200,-1))
        self.truthbutton = wx.Button(self, label="Add Truth Location", size=(200,-1))
        self.graphtext = wx.StaticText(self, label="Heat Maps:")
        self.approxtext = wx.StaticText(self, label="Approximations:")
        self.truthtext = wx.StaticText(self, label="Ground Truth:")

        self.savebutton = wx.Button(self, label="Save Current Graphic", size=(200,-1))

        self.Bind(wx.EVT_BUTTON, self.OnOk, self.btn1)
        self.Bind(wx.EVT_BUTTON, self.OnStop, self.btn2)
        self.Bind(wx.EVT_BUTTON, self.OnHeat, self.graphbutton)
        self.Bind(wx.EVT_BUTTON, self.OnApprox, self.approxbutton)
        self.Bind(wx.EVT_BUTTON, self.OnTruth, self.truthbutton)
        ...
        self.sizer = wx.GridBagSizer(15, 2)
        self.sizer.Add(self.line1, (0, 0))
        self.sizer.Add(self.line2, (0, 1))
        self.sizer.Add(self.line3, (0, 2))
        self.sizer.Add(self.line11, (6, 0))
        self.sizer.Add(self.line12, (6, 1))
        self.sizer.Add(self.line13, (6, 2))
        self.sizer.Add(self.line21, (10, 0))
        self.sizer.Add(self.line22, (10, 1))
        self.sizer.Add(self.line23, (10, 2))
        self.sizer.Add(self.runlabel, (1, 0))
        self.sizer.Add(self.runNum, (1, 1))

        self.sizer.Add(self.collarLabel, (3, 0))
        self.sizer.Add(self.collarList, (3, 1))
        self.sizer.Add(self.collarList2, (4, 1))
        self.sizer.Add(self.collarList3, (5, 1))

        self.sizer.Add(self.collarNum, (2, 0))
        self.sizer.Add(self.collarNumPicker, (2, 1))
        self.sizer.Add(self.collarbutton, (2, 2))

        self.sizer.Add(self.write, (4, 0))
        self.sizer.Add(self.advanced, (1, 2))

        self.sizer.Add(self.gauge, (7,2), flag=wx.ALIGN_CENTRE)
        self.sizer.Add(self.btn1,(7,0))
        self.sizer.Add(self.btn2,(7,1))
        self.sizer.Add(self.text,(8,2))

        self.sizer.Add(self.graphbutton,(11,2))
        self.sizer.Add(self.approxbutton,(12,2))
        self.sizer.Add(self.truthbutton,(13,2))
        self.sizer.Add(self.graphtext,(11,0))
        self.sizer.Add(self.approxtext,(12,0))
        self.sizer.Add(self.truthtext,(13,0))

        self.sizer.Add(self.savebutton,(15,2))
        ...
        # Set event handlers
        self.collarbutton.Bind(wx.EVT_BUTTON, self.OnCollarButton)
        self.advanced.Bind(wx.EVT_BUTTON, self.OnSettingsButton)

    def OnWrite(self,event):
        self.cfg = open('proc/CFG','w') 
        self.col = open('proc/COL','w') 
        self.job = open('proc/JOB','w')
        config = "center_freq: 172740000 \nsamp_rate: 10240 \ntimeout_interrupt: 1500 \nsignal_trigger_upper_percent: 90 \nsignal_trigger_lower_percent: 25 \nnumber_frames_per_file: 1 \ncurrentRunFile: " 
        self.run = self.runNum.GetValue()
        config = config + str(self.run)
        job = "/media/gilberto/RAW_DATA/rct/\ncurr_run: " + str(self.run)+"\n"
        colNum = self.collarNumPicker.GetValue()
        job+="freq_em: 15000 \npulse_ms: 20 \nlin_scale: 1 \nmap_d: 256 \nalpha_c_thres: 250 \nnum_col: "+str(colNum)
        col=""
        for colFreq in self.cArray:
            col+=str(colFreq)+"\n"
    ...
    def OnHeat(self,event):
        self.heatTrue = not self.heatTrue
        self.heat = cv.LoadImage('heat.png')
        self.state.add_object(SlipThumbnail('Heatmap', (32.8807161,-117.2319030), layer=1, img=self.heat, border_width=0, border_colour=(255,0,0)))

    import csv
    def OnApprox(self,event):
        filename = "META_" + str(self.run)+".csv"
        colors = [(255,0,0),(0,255,0),(0,0,255),(255,225,0),(255,0,255),(0,255,255)]
        fileM = open(filename,"rb")
        self.meta = csv.reader(fileM)
        for i,row in enumerate(self.meta):
            lat=int(row[1])/10000000.0
            lon=int(row[0])/10000000.0

            print str(lat) + "," + str(lon)
            self.state.add_object(SlipCircle('Approx'+str(self.run)+"_"+str(i), layer=1, latlon = (lat,lon), radius=.5, color=colors[i], linewidth=4))
            self.state.add_object(SlipCircle('ApproxR'+str(self.run)+"_"+str(i), layer=2, latlon = (lat,lon), radius=7, color=colors[i], linewidth=1))
        # self.state.add_object(SlipCircle('ApproxB', layer=1, latlon = (32.880253,-117.230580), radius=1, color=(0,255,0), linewidth=5))
        # self.state.add_object(SlipCircle('ApproxB2', layer=2, latlon = (32.880253,-117.230580), radius=7, color=(0,255,0), linewidth=1))
        # self.state.add_object(SlipCircle('ApproxG', layer=1, latlon = (32.879867,-117.229976), radius=1, color=(0,0,255), linewidth=5))
        # self.state.add_object(SlipCircle('ApproxG2', layer=2, latlon = (32.879867,-117.229976), radius=7, color=(0,0,255), linewidth=1))
        # self, key, layer, latlon, radius, color, linewidth 
        # self, key, latlon, layer, img,
        #          border_colour=None, border_width=0,
        #          popup_menu=nonlinear

    def OnTruth(self,event):
        self.state.add_object(SlipCircle('Heatmap', (32.8807161,-117.2319030), layer=1, img=self.heat, border_width=0, border_colour=(255,0,0)))
        # 
        # if(self.heatTrue):
        #     self.heat = Image.open('heat.png')
        #     self.state.add_object(SlipThumbnail('Heatmap', (32.8807161,-117.2319030), layer=1, img=self.heat, border_width=0, border_colour=(255,0,0)))
        # else:
        #     pass
            #self.state.clear_thumbnails()

    def OnOk(self, event):
        if self.count >= 50:
            return
        self.timer.Start(1000)
        self.text.SetLabel('Frequency Processing In Progress...')
        subprocess.call("./proc/spectrumAnalysis")
        self.text.SetLabel('Final Analysis In Progress...')
        subprocess.call("./proc/finalAnalysis")

    def OnStop(self, event):
        if self.count == 0 or self.count >= 50 or not self.timer.IsRunning():
            return
        self.timer.Stop()
        self.text.SetLabel('Processing Interrupted')
        wx.Bell()
    ...
    def OnTimer(self, event):
        ...
        if self.count == 50:
            self.timer.Stop()
            self.text.SetLabel('Processing Completed')

    def OnCollarButton(self, e):
        #self.result.SetLabel(self.editname.GetValue())
        self.CollDialog = CollarDialog(None, -1, "Update Collar Values", self.collarNumPicker.GetValue())
        #dialog = wx.MessageDialog(self, message = "Update Collar Values", caption = "SD Card", style = wx.YES_NO, pos = wx.DefaultPosition)
        self.cArray = []
        collarVal = ""
        collarVal2 = ""
        collarVal3 = ""
        for i,ob in enumerate(self.CollDialog.objects):
            if(i<=10):
                collarVal = collarVal + ob.GetValue() + "  "
            if((i>10) & (i<=20)):
                collarVal2 = collarVal2 + ob.GetValue() + "  "
            if((i>20) & (i<=30)):
                collarVal3 = collarVal3 + ob.GetValue() + "  "
            self.cArray.append(ob.GetValue())
        self.collarList.SetLabel(collarVal)
        self.collarList2.SetLabel(collarVal2)
        self.collarList3.SetLabel(collarVal3)
    def OnSettingsButton(self, e):
        sett = SettingsDialog(None, -1, "Advanced Settings")

    def OpenSD(self):
        SD_loc = 'F:/'
        if(os.path.isdir(SD_loc)):
            print "got SD"
        else:
            print "no SD"
...
class CollarDialog(wx.Dialog):
    def __init__(self, parent, id, title, numberCollars):
        numC = int(numberCollars)
        
        wx.Dialog.__init__(self, parent, id, title,size=(200, 100+25*numC))
        wx.StaticBox(self, -1, 'Collar Frequencies', (5, 5), size=(180, 25*numC+55))
        self.objects=[wx.TextCtrl(self, -1, pos=(20, 30), size=(140,-1))]
        for i in range(1, numC):
            ob=wx.TextCtrl(self, -1, pos=(20, 30+25*i), size=(140,-1))
            self.objects.append(ob)
        wx.Button(self, 1, 'Save', (60, 25*numC+30), (60, -1))
...
class SettingsDialog(wx.Dialog):
    def __init__(self, parent, id, title):
        ...
        self.box = wx.StaticBox(self, -1, 'Advanced Settings', pos=(5,5), size=(380, 210))
        
        self.pulseLabel = wx.StaticText(self, pos=(10,30), label="Pulse Length (ms):")
        self.pulseText = wx.TextCtrl(self, pos=(170,30), size=(140, -1), value="PULSE FROM FILE")
        
        self.freqLabel = wx.StaticText(self,pos=(10,60), label="Freq_em:")
        self.freqText = wx.TextCtrl(self, pos=(170,60),size=(140, -1), value="FREQ FROM FILE")
        
        self.mapLabel = wx.StaticText(self, pos=(10,90),label="Map_d:")
        self.mapText = wx.TextCtrl(self, pos=(170,90),size=(140, -1), value="MAP FROM FILE")
        
        self.alphaLabel = wx.StaticText(self, pos=(10,120),label="Alpha_c:")
        self.alphaText = wx.TextCtrl(self, pos=(170,120),size=(140, -1), value="ALPHA FROM FILE")
        
        self.linear = wx.CheckBox(self, -1, "Linear",pos=(10,150))
        #self.nonlinear = wx.CheckBox(self, -1, "Linear",pos=(170,150))

        self.save = wx.Button(self, 1, 'Save', pos=(170, 170), size=(60, -1))

        self.save.Bind(wx.EVT_BUTTON, self.OnClose, id=1)
...
class MPSlipMap():
    ...
    def child_task(self):
        ...
        self.app.frame = MainFrame(None, -1, 'Radio Collar Tracker',(0, 0), screenSize, state)
        self.app.frame.Show()
        self.app.MainLoop()
...
class MPSlipMapFrame(wx.Panel):
    ...
    def __init__(self, state, parent):
        ...
        #state.panel = MPInfoPanel(self,state)
...
class MPSlipMapPanel(wx.Panel):
    ...
    def change_zoom(self, zoom):
        ...
        state.ground_width = min(state.ground_width, 200000)
...
if __name__ == "__main__":
    ...
    parser.add_option("--lat", type='float', default=32.882802, help="start latitude")
    parser.add_option("--lon", type='float', default=-117.238411, help="start longitude")
